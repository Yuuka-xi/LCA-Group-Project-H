...
# =========================
# Railway: RLWT (timber sleeper track)
# =========================
events.rlwt <- c(
  rlwt_TS.full = 20,
  rlwt_SB.geo  = 100,
...
# =========================
# Explore ranges (RLWT vs STB) — follow the "successful logic"
# =========================
design.explore <- function(events1, events2) {
  ...
  for(i in 1:dim(events1)[1]) {
    ...
    dist.1 <- dist.Events(lifetime, ev1, start.Date, design.Options.rlwt)
    dur.ev1 <- dur.rlwt   # <-- just like success case
    
    for(j in 1:dim(events2)[1]) {
      ev2 <- unlist(events2[j, ])
      dist.2 <- dist.Events(lifetime, ev2, start.Date, design.Options.stb)
      dur.ev2 <- dur.stb  # <-- just like success case
...
# RLWT scenarios (Railway timber track) — fixed values + only FRR varies (65/66)
events.grid.RLWT <- expand.grid(
  rlwt_TS.full = 30,
  rlwt_SB.geo  = 90,
  FRR.rlwt     = sample(seq(65, 66, by = 1), n.grid, replace = TRUE)
)

# STB scenarios (Steel Truss Bridge)
...
# Run exploration
response.space <- design.explore(events.grid.RLWT, events.grid.STB)
