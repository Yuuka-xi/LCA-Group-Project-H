...
design.Options.pcf <- "PCF – Precast Concrete Facade"
design.Options.gcw <- "GCW – Glass Curtain Wall"
...
# PCF (Precast Concrete Facade)
events.Op1.PCF <- c(CR.pcf = 14, JM.pcf = 20, PR.pcf = 40, END = lifetime)
duration.ev.PCF <- c(CR.pcf = 3, JM.pcf = 5, PR.pcf = 7)


# GCW 
events.Op1.GCW <- c(DC.gcw = 7, GR.gcw = 10, IR.gcw = 40, END = lifetime)
duration.ev.GCW <- c(DC.gcw = 3, GR.gcw = 5, IR.gcw = 14)
...
n.grid <- 3
# PCF scenarios
# PCF scenarios (parameter range exploration)
events.grid.PCF <- expand.grid(
  CR.pcf = seq(12, 18, by = 1),
  JM.pcf = seq(15, 25, by = 1),
  PR.pcf = seq(40, 60, by = 10)
)

# GCW scenarios
events.grid.GCW <- expand.grid(
  DC.gcw = seq(5, 10, by = 1),
  GR.gcw = seq(8, 14, by = 1),
  IR.gcw = seq(40, 60, by = 10)
)
