...
# Convert environmental impacts into monetary Cost
Energy.Cost <- 0.081
...
fitness <- function(x) {
  ...
  integrated.system <- mutate(
    integrated.system,
    TotalCost = (Energy * Energy.Cost + CO2/1000 * CO2.unitcost + NOx/1000 * NOx.unitCost +  product.1.pcf$Cost +
               product.2.gcw$Cost +
               product.3.brc$Cost +
               product.4.stb$Cost +
               product.5.rlwc$Cost +
  ...
  z[6] <- integrated.system$Cost
  z[7] <- integrated.system$TotalCost
...
outNames <- c("duration", "interv.dist", "energy", "co2", "nox", "cost", "TotalCost")
...
colnames(all.results) <- c(
  ...
  "duration","interv.dist","energy","co2","nox","cost", "TotalCost",
  "pareto"
)
...
top_5_best <- head(pareto_only[order(pareto_only$TotalCost), ], 1)
top_environment <- head(pareto_only[order(pareto_only$cost), ], 1)
top_duration <- head(pareto_only[order(pareto_only$duration), ], 1)
# Ver los intervalos de mantenimiento de las 5 mejores soluciones
print(top_5_best[, 1:24])
print(top_environment[, 1:24])
print(top_duration[, 1:24])