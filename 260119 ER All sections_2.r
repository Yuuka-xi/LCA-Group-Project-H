...
fitness <- function(x) {
  ...
  y <- expand.grid( #Yuuka: We deleted some events, so maybe you need to edit this part later?
  ...
  #Yuuka: We have defined the duration for each events, perhaps we don't need to use half-duration assumption
  # --------------------------------
  ...
  #Yuuka: added option name or there will be warning
  dist.1 <- apply(y[1:3], 1, function(ev)
    dist.events(lifetime = lifetime, events = ev, start.Date = start.Date,
                option.Name = "PCF", do.plot = FALSE)
  )
  
  dist.2 <- apply(y[4:6], 1, function(ev)
    dist.events(lifetime = lifetime, events = ev, start.Date = start.Date,
                option.Name = "GCW", do.plot = FALSE)
  )
  
  dist.3 <- apply(y[7:9], 1, function(ev)
    dist.events(lifetime = lifetime, events = ev, start.Date = start.Date,
                option.Name = "BRC", do.plot = FALSE)
  )
  
  dist.4 <- apply(y[10:14], 1, function(ev)
    dist.events(lifetime = lifetime, events = ev, start.Date = start.Date,
                option.Name = "STB", do.plot = FALSE)
  )
  
  dist.5 <- apply(y[15:17], 1, function(ev)
    dist.events(lifetime = lifetime, events = ev, start.Date = start.Date,
                option.Name = "RLWC", do.plot = FALSE)
  )
  
  dist.6 <- apply(y[18:21], 1, function(ev)
    dist.events(lifetime = lifetime, events = ev, start.Date = start.Date,
                option.Name = "RLWT", do.plot = FALSE)
  )
  ...
  # -------------------------------------------------------
  # Yuuka: I think we don't need the data, we already have them in the event_input table
  ...
  product.3.brc  <- LCA.byEvents(events_input.BRC,  dist.3[[1]], LCI.materials)
